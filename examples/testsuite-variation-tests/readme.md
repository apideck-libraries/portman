...
This example contains the setup of Portman variation test generation, which will convert the OpenAPI document to a Postman collection, while adding Postman tests. The generated tests are focussed on the variations of API contracts.
...
- **unhappy path variations** . We verify if the API response is matching with the expected invalid or error response. Typically, by entering invalid input in the request body/headers/query params this would mean 4xx, 5xx responses
...
The following variations tests can be inject by using the following properties:
...
- **variations (Array )** : References to list of variations that will be injected
  ...
  - **tests** : which refers the definitions for the specific tests for the variation.
...
We instruct Portman to generate variations.
...
The overwrites contains the list of all the overwrite you want to do on the targeted `leadsApp`.
...
Variance support all test types:
...
The contract tests will be injected for this specific `leadsApp`varation.
...
The content tests will be added for the targeted `leadsApp`varation.

In our example, we added the content test since we want to verify that the response body contains the `resource` property with the value `leads`
...
The extend tests is an additional test inject that can used for the targeted `leadsApp`varation.

In our example, we added a custom postman test, that will be added after all the other tests.
...
When using variantions, it can also be useful to assign some variables based on the response/request to be used afterwards in the Postman collection.
