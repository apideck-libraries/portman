...
describe('TestSuite', () => {
  ...
  beforeEach(async () => {
    ...
    config = await getConfig(portmanVariationConfigFile)
    testSuiteServiceForVariation = new TestSuite({ oasParser, postmanParser, config })
  })
  ...
  it('should generateContractTests for variations', () => {
    testSuiteServiceForVariation.generateContractTests()
    expect(omitKeys(testSuiteService.collection.toJSON(), ['id', '_postman_id'])).toMatchSnapshot()
  })
