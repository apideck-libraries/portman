...
export class PostmanDynamicVarGenerator {
  ...
  constructor() {
    ...
    this.dynamicGenerators = {
      ...
      // faker's system.filePath returns nothing. this returns a path for a file.
      // $randomFilePath: {
      //   description: 'A random file path',
      //   generator: function () {
      //     return (
      //       this.dynamicGenerators.$randomDirectoryPath.generator() + '/' + faker.system.fileName()
      //     )
      //   }
      // },
      ...
      $randomIntTest: {
        description: 'Return 123 (for testing purposes',
        generator: function () {
          return 123
        }
      }
  ...
  public dynamicGeneratorsRegex(): RegExp {
    let generatorRegexString = ''
    Object.entries(this.dynamicGenerators).forEach(([key]) => {
    ...
    return new RegExp(generatorRegexString, 'g')
  }

  public replaceDynamicVar(value: string): string {
    const dynamicRegex = this.dynamicGeneratorsRegex()
    return value.replace(dynamicRegex, match => {
      // We remove the enclosing {{ }} to extract the generator type like "$randomName"
      const generatorType = match.slice(2, -2)
      return this.dynamicGenerators[generatorType].generator()
    })
  }

  public renderDynamicVar(dynamicVariable: string): any {
    const functionName = '$' + dynamicVariable
    if (this.dynamicGenerators[functionName]) {
      return this.dynamicGenerators[functionName].generator()
    } else {
      throw new Error(`Unsupported Postman dynamic variable: ${dynamicVariable}`)
    }
  }
}
